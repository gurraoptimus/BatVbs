...
ping -n 2 8.8.8.8>%desktop%\ping.txt
for /f "tokens=4 delims==" %%a in ('type %desktop%\ping.txt ^| find "Average"') do set ping=%%a
for /f "tokens=4 delims==" %%a in ('type %desktop%\ping.txt ^| find "Lost"') do set ploss=%%a
